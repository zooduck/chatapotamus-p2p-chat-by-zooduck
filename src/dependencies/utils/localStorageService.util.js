...
/**
 * Proxy the localStorage object with traps to allow namespacing of keys
 * and provide eventing capability (for a "setitem" event).
 * @example
 * // Set an item with an expiry date of 10 minutes:
 * const expiryDate = new Date()
 * expiryDate.setMinutes(expiryDate.getMinutes() + 10)
 * localStorageService.setItem('abc', 123, { expires: expiryDate })
 * @example
 * // Remove all items (only keys set using this proxy service will be removed from localStorage!):
 * localStorageService.clear()
 * @example
 * // Using the onsetitem event handler property:
 * localStorageService.onsetitem = (event) => {}
 * @example
 * // Using addEventListener():
 * localStorageService.addEventListener('setitem', (event) => {})
 */
  ...
  get (target, property) {
    ...
    if (value instanceof Function) {
      return function(...args) {
        if (value.name === 'clear') {
          // ---------------------------------------------
          // Only clear namespaced keys from localStorage
          // ---------------------------------------------
