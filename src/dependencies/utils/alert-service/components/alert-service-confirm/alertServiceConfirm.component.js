...
class HTMLAlertServiceConfirmElement extends WebComponent {
  ...
  onKeyDown(event) {
    ...
    if (!this.#focussedPromptButton && isEnterKey && !this.contained) {
      // -------------------------------------------------------------
      // If the confirm text is selected, focus will have shifted from
      // one of the prompt buttons to the selected text but we
      // still want to close the confirm if the Enter key is pressed.
      // -------------------------------------------------------------
      this.sendCustomEvent(this.constructor.EventDict.OK);
      this.remove();
