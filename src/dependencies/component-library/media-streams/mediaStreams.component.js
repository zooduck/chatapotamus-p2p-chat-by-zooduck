...
class HTMLMediaStreamsElement extends WebComponent {
  ...
  /**
   * @method
   * @param {string} mediaStreamSenderID
  ...
  /**
   * If we use the "autoplay" attribute on the video, we get a brief flash
   * of the video poster before the video plays. To prevent this, I am
   * using the "canplay" event to delay showing the video element until it
   * has started playing (using an arbitrary delay of 250ms).
   *
   * @type {EventListener}
   */
  ...
  #attachDisplay(mediaStreamSenderID, mediaStream) {
    ...
    if (mediaStreamHasVideo) {
      videoStream = mediaStream; // This can include audio
    } else {
